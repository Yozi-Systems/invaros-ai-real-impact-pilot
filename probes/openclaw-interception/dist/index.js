...
function requireSecureFile(fd, expectedPath) {
    ...
    return {
        ...
        mode: Number(fileStat.mode & 511n)
    };
}
...
function requireUnchangedFile(file) {
    ...
    if (!current.isFile() ||
        ...
        Number(current.mode & 511n) !== file.mode) {
    ...
    register(api) {
        ...
        try {
            ...
            api.on("before_tool_call", async (event) => {
                if (event.toolName !== PROBE_TOOL_NAME)
                    return;
                ...
                if (config.fault === "rewrite") {
                    if (event.params.marker !== "allow-case") {
            ...
            api.registerTool({
                ...
                async execute(toolCallId, params) {
                    ...
                    const markerIsValid = config.fault === "rewrite"
